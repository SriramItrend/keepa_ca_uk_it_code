...
def create_tables():
    ...
    cursor = conn.cursor()
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS modified_price_history_ca (
        date DATE,
        asin VARCHAR(255),
        Price FLOAT
    );""")
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS modified_category_rank_history_ca (
        date DATE,
        asin VARCHAR(255),
    ...
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS modified_sub_category_rank_history_ca (
        date DATE,
        asin VARCHAR(255),
        category_type VARCHAR(255),
...
category_query = """
SELECT
    DATE(category_rank_history_CA.`date`) AS `date`,
    CASE
        WHEN category_rank_history_CA.category IN (
            'Automotive',
            'Patio, Lawn & Garden',
            'Baby Products',
    ...
    category_rank_history_CA.category,
    category_rank_history_CA.asin,
    ROUND(MIN(category_rank_history_CA.`rank`)) AS avg_rank
    FROM
    category_rank_history_CA
GROUP BY
    DATE(category_rank_history_CA.`date`),
    category_type,
    category_rank_history_CA.category,
    category_rank_history_CA.asin;
"""
...
price_query = """
SELECT
    DATE(new_price_history_CA.`date`) AS `date`,
    new_price_history_CA.asin,
    ROUND(MAX(new_price_history_CA.price), 2) AS Price
FROM
    new_price_history_CA
GROUP BY
    DATE(new_price_history_CA.`date`),
    new_price_history_CA.asin;
"""
...
tables_to_truncate = ['modified_price_history_ca', 'modified_category_rank_history_ca', 'modified_sub_category_rank_history_ca']
truncate_tables(tables_to_truncate)


# Insert data into local MySQL database using retry mechanism
insert_with_retries(final_price_df, 'modified_price_history_ca', engine)
insert_with_retries(final_category_df, 'modified_category_rank_history_ca', engine)
insert_with_retries(final_sub_category_df, 'modified_sub_category_rank_history_ca', engine)
